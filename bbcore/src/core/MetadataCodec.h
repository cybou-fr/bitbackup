...
//
// Дополнение обязательно. Открытого заголовка у чанка нет (§16), но размер
// объекта хранилище видит всегда, и по нему без выравнивания оценивались бы
// длина пути и число чанков — merkle_path растёт как ⌈log₂(n)⌉ × 32.
// Три класса вместо точной длины сводят эту утечку к выбору из трёх значений.
//
...
struct ChunkMetadata {
    ...
    /// Длина случайного дополнения в конце объекта (§16). В suite 1 всегда 0.
    /// Лежит здесь, а не в открытом виде, потому что открытых полей у
